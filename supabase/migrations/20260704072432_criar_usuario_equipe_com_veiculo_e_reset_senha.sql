drop function if exists public.criar_usuario_equipe(text,text,text,text,text);
...
-- Dono consegue redefinir a senha de qualquer colaborador (ex: motoboy esqueceu o PIN).
create or replace function public.redefinir_senha_usuario(p_usuario_id uuid, p_nova_senha text)
returns boolean
language plpgsql
security definer
set search_path to 'public', 'extensions'
...
begin
  ...
  if v_chamador is null or v_chamador.role <> 'dono' then
    raise exception 'Só o dono pode redefinir senhas.';
  end if;
...
$$;
grant execute on function redefinir_senha_usuario(uuid, text) to authenticated;
