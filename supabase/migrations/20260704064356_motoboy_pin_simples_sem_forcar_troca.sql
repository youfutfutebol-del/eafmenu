...
begin
  ...
  if p_role = 'motoboy' then
    -- Senha simples (PIN de 4 dígitos), definida pelo dono, sem forçar troca no primeiro acesso.
  ...
  values (v_novo_id, v_chamador.restaurante_id, p_nome, nullif(trim(p_email),''), trim(p_telefone), p_role, v_primeiro_acesso);
