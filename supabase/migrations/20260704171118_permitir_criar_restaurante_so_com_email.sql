...
begin
  ...
  -- Agora aceita telefone OU e-mail (antes exigia telefone sempre)
  if (p_dono_telefone is null or trim(p_dono_telefone) = '') and (p_dono_email is null or trim(p_dono_email) = '') then
    raise exception 'Informe pelo menos o telefone ou o e-mail do dono.';
  end if;
  ...
  values (v_dono_id, v_restaurante_id, p_dono_nome, v_email_final, nullif(trim(p_dono_telefone), ''), 'dono', false);
