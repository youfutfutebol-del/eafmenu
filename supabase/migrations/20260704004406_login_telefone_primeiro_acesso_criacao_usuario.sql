-- 1) Coluna de "primeiro acesso" (força troca de senha no próximo login)
alter table usuarios add column if not exists primeiro_acesso boolean not null default true;
-- Conta que já está em uso (Eduardo/Dono Teste) não precisa ser forçada a trocar senha
update usuarios set primeiro_acesso = false where primeiro_acesso is distinct from false;

-- 2) Resolve login por telefone OU e-mail, devolvendo o e-mail real (usado internamente pelo Supabase Auth)
...
begin
  if p_identificador ilike '%@%' then
    return lower(trim(p_identificador));
  end if;

  select email into v_email
  from usuarios
  where telefone = trim(p_identificador) and ativo = true
  limit 1;
...
-- 3) Cria um novo usuário de equipe (auth + usuarios) com senha provisória, tudo dentro do Supabase.
-- Só dono/gerente podem chamar; gerente não pode criar outro gerente ou dono.
...
begin
  ...
  values (v_novo_id, v_chamador.restaurante_id, p_nome, nullif(trim(p_email),''), trim(p_telefone), p_role, true);
