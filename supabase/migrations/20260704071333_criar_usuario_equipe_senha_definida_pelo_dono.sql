drop function if exists public.criar_usuario_equipe(text,text,text,text);
...
begin
  ...
  -- Se o dono digitou uma senha, usa ela. Senão, gera uma (fallback).
  if p_senha is not null and length(trim(p_senha)) >= 4 then
    v_senha := trim(p_senha);
    v_primeiro_acesso := false; -- o dono já escolheu a senha de propósito, não precisa forçar troca
...
$$;
grant execute on function criar_usuario_equipe(text,text,text,text,text) to authenticated;
