...
declare
  ...
  v_candidato record;
begin
  select id, role, restaurante_id
  into v_chamador
  from usuarios
  where id = auth.uid();

  if v_chamador is null
     or v_chamador.role not in ('dono', 'gerente', 'atendente')
  then
    raise exception 'Você não tem permissão para cancelar pedidos.';
  end if;

  select id, restaurante_id, status, finalizado_em
  into v_pedido
  from pedidos
  where id = p_pedido_id;

  if v_pedido is null
     or v_pedido.restaurante_id <> v_chamador.restaurante_id
  then
  ...
  if v_pedido.status in ('entregue', 'retirado') then
    if v_pedido.finalizado_em is null then
      raise exception
        'Pedido finalizado sem data válida de finalização. Contate o suporte.';
    end if;

    if v_pedido.finalizado_em > now() then
      raise exception
        'Pedido finalizado sem data válida de finalização. Contate o suporte.';
    end if;
    ...
    if now() > v_pedido.finalizado_em + interval '24 hours' then
      raise exception
        'Pedidos finalizados só podem ser cancelados nas primeiras 24 horas.';
    end if;
  end if;
  ...
  if p_senha_confirmacao is null
     or length(trim(p_senha_confirmacao)) = 0
  then
  ...
  if v_chamador.role in ('dono', 'gerente') then
    select exists (
      ...
      where id = v_chamador.id
        and encrypted_password =
            crypt(p_senha_confirmacao, encrypted_password)
    )
    into v_senha_ok;
    ...
    for v_candidato in
      ...
      where u.restaurante_id = v_chamador.restaurante_id
        ...
        and u.ativo = true
    loop
      if v_candidato.encrypted_password =
         crypt(
           p_senha_confirmacao,
           v_candidato.encrypted_password
         )
      then
